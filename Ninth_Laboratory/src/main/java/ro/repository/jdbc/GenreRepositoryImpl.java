 ...
 * @author Alex Neagu
 */
public class GenreRepositoryImpl extends BaseController implements GenreRepository {
    ...
    private static final String GET_COUNT = "select count(*) from genres";

    public GenreRepositoryImpl(Connection con) {
        super(con);
    }
    ...
    @Override
    public Long getGenreCount() throws FindException {
        try (PreparedStatement preparedStatement = con.prepareStatement(GET_COUNT)) {
            ResultSet resultSet = preparedStatement.executeQuery();
            if (!resultSet.next()) {
                throw new SQLException("Genre count couldn't be calculated");
            }
            return resultSet.getLong("count");
        } catch (SQLException ex) {
            throw new FindException("getCount", ex.getMessage(), "genres");
        }
    }
}
